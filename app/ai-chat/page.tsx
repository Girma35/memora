      ...
      <div className="flex flex-1 w-full max-w-[1400px] mx-auto mt-8 px-6 gap-8 h-[calc(100vh-80px)]">
          ...
          <div className="flex flex-col gap-8 flex-1 overflow-y-auto pb-32 no-scrollbar">
            ...
            <div className="flex flex-col items-end">
              <div className="bg-[#2A2A2A] text-white px-6 py-4 rounded-2xl rounded-tr-sm max-w-[80%] border border-white/5">
                <p className="text-sm">What deployment bug did I solve last week?</p>
              </div>
              <span className="text-[10px] text-zinc-500 mt-2 mr-2">10:42 AM</span>
            </div>
            ...
            <div className="flex gap-4 max-w-[85%]">
              ...
              <div className="flex flex-col gap-4 w-full">
                <p className="text-sm text-zinc-300 leading-relaxed">
                  Based on your Session Timeline, you resolved a critical hydration mismatch in <code className="bg-white/10 px-1.5 py-0.5 rounded text-xs">auth.ts</code> last Thursday.
                </p>
                <p className="text-sm text-zinc-300 leading-relaxed">
                  The issue occurred during the initial server-side render where the authentication token state did not match the client's local storage state, causing a flash of unauthenticated content.
                </p>
                ...
                <div className="rounded-xl bg-black/40 border border-white/5 overflow-hidden mt-2">
                  <div className="px-4 py-3 border-b border-white/5 flex items-center justify-between">
                    <div className="flex items-center gap-2 text-sm text-white font-medium">
                      <History className="size-4 text-[#00E5FF]" /> Session Timeline Reference
                    </div>
                    <Button variant="ghost" size="icon" className="size-6 text-zinc-400 hover:text-white">
                      <ExternalLink className="size-3" />
                    </Button>
                  </div>
                  <div className="p-4">
                    <p className="text-sm text-white mb-2">Deployment Hotfix #892</p>
                    <div className="flex gap-4 text-xs text-zinc-500">
                      <span className="flex items-center gap-1.5"><Clock className="size-3" /> Oct 12, 14:30</span>
                      <span className="flex items-center gap-1.5 font-mono">{"<>"} auth.ts, _app.tsx</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="absolute bottom-0 left-0 right-8 pb-8 pt-6 bg-gradient-to-t from-[#111111] via-[#111111] to-transparent">
            <div className="relative flex items-center bg-[#1A1A1A] rounded-full border border-white/10 p-2 shadow-2xl">
              ...
              <div className="flex items-center gap-1 pr-1">
                ...
                <Button size="icon" className="size-10 rounded-full bg-[#1A2629] hover:bg-[#1A2629]/80 text-[#00E5FF]">
        ...
        <aside className="w-80 shrink-0 flex flex-col gap-8 h-full overflow-y-auto pb-8">
          ...
          <div className="flex flex-col gap-3">
            <p className="text-[10px] font-bold text-zinc-500 tracking-widest uppercase">Relevant Files</p>
            <div className="flex flex-col gap-2">
              <Button variant="outline" className="w-full justify-start h-auto py-3 bg-[#161616] border-white/5 hover:bg-white/5 hover:border-white/10 text-zinc-300 font-normal">
                <FileText className="size-4 mr-2 text-zinc-500" /> auth.ts
              </Button>
              <Button variant="outline" className="w-full justify-start h-auto py-3 bg-[#161616] border-white/5 hover:bg-white/5 hover:border-white/10 text-zinc-300 font-normal">
                <FileText className="size-4 mr-2 text-zinc-500" /> session_provider.tsx
              </Button>
            </div>
          </div>

          <div className="flex flex-col gap-3">
            <p className="text-[10px] font-bold text-zinc-500 tracking-widest uppercase">Related Sessions</p>
            <div className="flex flex-col gap-2">
              <div className="rounded-lg bg-[#161616] border border-white/5 p-4 hover:border-white/10 transition-colors cursor-pointer">
                <p className="text-sm text-zinc-300 mb-1">Auth Hydration Debugging</p>
                <p className="text-xs text-zinc-500">Oct 11 • 2.5 hrs</p>
              </div>
              <div className="rounded-lg bg-[#161616] border border-white/5 p-4 hover:border-white/10 transition-colors cursor-pointer">
                <p className="text-sm text-zinc-300 mb-1">NextJS Middleware Setup</p>
