...
const initialIntegrations: Integration[] = [
	{
		...
		description:
			"Monitoring active workspace, syntax state, and local file changes.",
		status: "connected",
	...
	},
	{
		...
		description:
			"Track commits, pull requests, and branch activity automatically.",
		status: "disconnected",
	...
	},
	{
		...
		subtitle: "Chrome, Arc",
		description:
			"Captures web research, active tabs, and reading list contexts.",
		status: "active",
		statusLabel: "Active",
		actionLabel: "Configure",
		actionVariant: "outline",
		...
		configFields: [
			{
				label: "Capture Mode",
				placeholder: "tabs, history, bookmarks",
				value: "tabs, history",
			},
			{
				label: "Excluded Domains",
				placeholder: "mail.google.com, ...",
				value: "mail.google.com, bank.com",
			},
	...
	},
	{
		...
		subtitle: "Zsh, Bash",
		description:
			"Track shell history, executed commands, and environment state.",
		status: "disconnected",
		statusLabel: "Disconnected",
		actionLabel: "Install",
		...
		configFields: [
			...
			{
				label: "Capture Environment Vars",
...
function StatusBadge({
	...
	return (
		<span
			className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-xs font-medium ${styles[status]}`}
		>
			...
			prev.map((i) =>
				i.id === id
					? {
							...
							statusLabel: "Disconnected",
							actionLabel: i.requiresCli ? "Install" : "Connect",
							actionVariant: i.requiresCli ? "install" : "primary",
							lastSync: undefined,
						}
					: i,
			),
				...
				<main className="flex-1">
					...
					<div className="grid grid-cols-3 gap-4 mb-8">
						{[
							{
								label: "Connected",
								value: integrations.filter((i) => i.status !== "disconnected")
									.length,
							...
							{
								label: "Total Sources",
								value: integrations.length + 1,
								icon: <Zap className="size-4 text-[#00E5FF]" />,
								color: "text-[#00E5FF]",
							},
						].map((stat) => (
							<div
								key={stat.label}
								className="rounded-xl border border-white/5 bg-[#141414] p-4 flex items-center gap-4"
							>
								...
								<div className="flex items-center justify-between mt-auto pt-2 border-t border-white/5">
									<div className="text-xs text-zinc-600">
										...
										{integration.requiresAuth && (
											<span className="text-amber-500/80">Requires Auth</span>
										)}
										{integration.requiresCli && (
									...
									<div className="flex items-center gap-2">
										...
										{integration.status !== "disconnected" && (
											...
											>
												<RefreshCw
													className={`size-3.5 ${syncing === integration.id ? "animate-spin text-[#00E5FF]" : ""}`}
												/>
											</button>
										)}
					...
					<div className="rounded-2xl border border-dashed border-white/10 bg-[#141414]/50 p-10 flex flex-col items-center justify-center text-center">
						...
						<p className="text-zinc-500 text-sm max-w-sm mb-5">
							Upcoming integration to index conversations and track async
							decisions across your team&apos;s channels.
						</p>
